...
def getLBPImage(image):
    ...
    lbp_value = local_binary_pattern(image, 8, 1)

    shaped = blockshaped(lbp_value, 16, 16)

    xBlocks = []
    for s in shaped:
        xBlocks.append(histogram(s))
    
    return np.concatenate(xBlocks)
...
crop_dir = './lwf/lwf_cropped'
path_list, issame_list = get_paths(crop_dir, pairs)

embeddings = get_embedding(path_list)
...
# Accuracy:  0.638
# Validation Rate:  0.05433333333333333
# False Accept Rate:  0.0013333333333333335